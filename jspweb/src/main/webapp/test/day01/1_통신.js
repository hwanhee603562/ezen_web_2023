...
function 예제1(){
	...
	/*
		AJAX : JQUERY 라이브러리에서 통신 HTTP 함수 제공
			JQUERY : JS언어로 여러 개 함수를 제공하는 라이브러리
		1. 정의 : JS언어에서 외부와 통신( 데이터 주고받기 )하는 목적
		2. 형태 : $.ajax();
		3. 필수 : JQUERY 라이브러리 필수!
				<script src="http://code.jquery.com/jquery-latest.min.js"></script>
				$ is not defined : jquery 라이브러리 추가 안했을 때 발생하는 오류
	...
	*/
	...
	$.ajax({ 
		...
		success : function( result ){ console.log(result); }		// 3. 통신을 성공했을 때 반환된 데이터
...
function 예제2(){
	console.log('예제2() 실행')
	 ...
	 $.ajax( { 
      url : "http://192.168.17.17//jspweb/Test01"  ,    	// 1. 통신할 경로
      method : "get",                               		// 2. 통신할 HTTP 메소드 방식 
      success : function 결과( result ){ console.log(result); }   // 3. 통신을 성공했을때 반환된 데이터
   });
