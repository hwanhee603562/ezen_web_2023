...
public class MemberFindController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result) {
			...
			MemberDto dto = ( MemberDto )request.getSession().getAttribute("loginDto");
				// 저장된 데이터 타입이 object 타입이기 때문에 강제타입변환을 통해 호출해야함
			System.out.println( "세션상태확인 : "+dto );
				// 출력 => MemberDto [mno=5, mid=azz258, mpwd=null, memail=abb258@naver.com, mimg=bfs3.png]
		}
	...
	// 하나의 메소드의 여러개 ajax 통신할때. type전송
	// ( 1:아이디중복검사 2.이메일중복검사 vs 필드명 = mid : 아이디중복검사, memail : 이메일중복검사 )
