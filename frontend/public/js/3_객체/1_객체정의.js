/**
	...
	JS : 객체 지향 언어[ OOP : Object Oriented Programming ]
		...
		대량화/자동화 ===> 객체지향 프로그래밍 적합하다.
		 => 미리 설계(클래스)하고 컴퓨터 설꼐 주입해서 설계기반으로 형태(객체) 생성
	...
	JS 객체 = JSON
		1. JS 메모리
			...
			2. 상수 = 데이터
			3. 배열 = [ 데이터1, 데이터2, 데이터3 ]
				- 배열단점 : 서로 다른 정보유형들을 하나의 배열에 관리하기 어려움
					ex) 유재석/강호동/신동엽 3명의 회원(정보:아이디, 비밀번호, 이름, 주소 등) 등록
						1 방법 : 정보유형 분류[ 데이터 유형이 아이디인지 비밀번호인지 식별 ]
							...
							주소배열 = []
						2 방법 : 인덱스 기준 분류
							회원배열 [ 'qwe', 'qwe', '안산', '강호동', '경기도' ]
						3 방법 : 객체유형
							let 회원1 = { 아이디 : 'qwe', 비밀번호 : 'qwe', 이름 : '유재석', 주소 : '안산' }
							let 회원2 = { 아이디 : 'adz', 비밀번호 : 'asd', 이름 : '강호동', 주소 : '수원' }
							let 회원3 = { 아이디 : 'qqsx', 비밀번호 : 'zxc', 이름 : '신동엽', 주소 : '성남' }	
							
							
			4. 객체 = {} 
				- 1. 사용목적 : 서로 다른 데이터유형(속성)들을 한 곳에 저장 / 묶음
				- 2. 형태
					let 객체명 = { 속성명 : 데이터, 속성명 : 데이터 }
				- 3. 선언
					키워드 객체명 = 	{
										속성명 : 데이터,
										속성명 : 변수,
...
// 2. 객체 호출
console.log( 객체1 )
console.log( '아이디속성 호출 : '+ 객체1.아이디 )
console.log( '비밀번호속성 호출 : '+ 객체1.비밀번호 )
	// console : 내장객체		-> console객체에서 log()함수 호출
	// console : 내장객체		-> document객체에서 qeuerySelector() 함수 호출
	//vs
console.log( 배열1[0] )

// 3. 객체의 속성내 데이터 변경
객체1.비밀번호 = '123456'	// [변경] 객체내 새로운 값으로 대입
